...
- 무작정 Entity를 만들어 사용하는 것보다 ERD를 설계할 때 각 테이블의 역할에 대해서 좀 더 깊게 고민하는 계기가 되었습니다.

### 5.2 Toast UI Editor 이미지 업로드 처리
- 저는 게시글을 작성할 떄 여러 장의 이미지를 업로드할 수 있도록 처리하고 싶었습니다. 
- 이미지를 업로드하는 흐름은 이러합니다. 
...
- 이 과정에서 프론트 측의 상태 관리 이슈가 있었습니다. Editor 부분이 React의 Return문에 들어있었기에 여러 장의 이미지를 업로드할 때마다 상태관리는 초기화되어서 추가적인 처리가 필요했고 이는 Client에게 너무 많은 짐을 지는 것 같았습니다. 또 이미지를 수정할 때 이미지의 사용 여부를 판단할 방법이 부족했습니다. 
- 결국 Server에서 이미지의 사용여부를 직접 체크하도록 서비스 계층에서 메서드를 작성하였습니다. 게시글은 Markdown 형식으로 전달된다는 전제하에 작성하였습니다.



### 5.3 WebSocket 실시간 알림 구현 방식
